...
export default function Dashboard() {
  ...
  const myAnnouncements = useMemo(
    () => announcements.filter(a => a.conductorId === user?.id),
    [announcements, user]
  );
  ...
  // Count enrollments for conductor's classes from dummyEnrollments
  const myClassIds = new Set(myClasses.map(c => c.id));
  const enrolledStudents = dummyEnrollments
  ...
  const stats = [
    ...
    { label: 'Active Notices',value: activeAnns,       icon: '📢', colorVal: 'text-violet-600',   colorBg: 'bg-violet-50 border-violet-200' },
  ];
  ...
  const actions = [
    ...
    { icon: '👤', label: 'Edit Profile',        desc: 'Update your info',                 to: '/conductor/profile',       primary: false },
  ];
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-sub text-sm mt-1">
          Here's a snapshot of your activity on KuppiConnect.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {stats.map(s => (
          <div
            key={s.label}
            className={`bg-white border rounded-2xl p-5 shadow-sm ${s.colorBg.split(' ')[1]}`}
          >
        ...
        <div className="space-y-3">
          {myClasses.slice(0, 4).map(cls => {
            ...
            return (
              <div
                key={cls.id}
                className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm flex items-center gap-4 hover:border-primary/20 transition-all"
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-3 mb-1">
                    <div>
                      ...
                      <p className="text-dim text-xs mt-0.5">{cls.date} · {cls.time} · {cls.location || cls.meetingLink}</p>
                    </div>
                    <span className={`text-xs font-semibold px-2.5 py-1 rounded-full shrink-0 ${
                      isFull ? 'bg-red-50 text-err' : 'bg-green-50 text-ok'
                    }`}>
                      {isFull ? 'Class Full' : `${(cls.seats ?? 0) - (cls.enrolled ?? 0)} seats left`}
                    </span>
                  </div>
                ...
                <div className="flex gap-2 shrink-0">
                  <Link
                    ...
                    className="text-xs font-semibold text-primary bg-sky-50 border border-sky-200 px-3 py-2 rounded-xl hover:bg-sky-100 transition-all"
                  >
                    Edit
                  </Link>
                </div>
